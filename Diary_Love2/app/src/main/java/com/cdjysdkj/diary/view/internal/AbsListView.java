package com.cdjysdkj.diary.view.internal;
...
//			}
	...
	protected float getBottomFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			final float fadeLength = (float) getVerticalFadingEdgeLength();
			return bottom > height - getPaddingBottom() ? (float) (bottom - height + getPaddingBottom())
					/ fadeLength : fadeEdge;
		}
	}
...
//		Log.d("TAG", "spaceBelow is " + spaceBelow + " , spaceAbove is " + spaceAbove);
		...
		Log.d("TAG", "incrementalDeltaY is " + incrementalDeltaY + " spaceabove is " + spaceAbove + " space below is " + spaceBelow);
		final int firstPosition = mFirstPosition;
		...
		if (firstPosition + childCount == mItemCount && endBottom <= end && deltaY <= 0) {
			...
			// is already visible
//			Log.d("TAG", "return last bottom is " + higherBottom + ", end is  " + end);
			return true;
		}
		...
		if (down) {
			...
			for (int i = 0; i < childCount; i++) {
				final View child = getChildAt(i);
//				Log.d("TAG", "scroll down top is " + top + " , view " + i + " bottom is " + child.getBottom());
...
//						Log.d("TAG", "add to scrap view");
			...
			for (int i = childCount - 1; i >= 0; i--) {
				final View child = getChildAt(i);
//				Log.d("TAG", "scroll down bottom is " + bottom + " , view " + i + " top is " + child.getTop());
...
//			Log.d("TAG", "detach view from " + start + " , count is " + count);
		}
	 ...
	 */
	abstract void fillGap(boolean down, int startOffset);
	...
	private void positionPopup() {
		...
		if (!mPopup.isShowing()) {
			mPopup.showAtLocation(this, Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, xy[0],
					bottomGap);
		} else {
...
//				Log.d("TAG", "get scrap view size is " + size);
				if (size > 0) {
...
//				Log.d("TAG", "add scrap view size is " + mCurrentScrap.size());
			} else {
				dispatchFinishTemporaryDetach(scrap);
