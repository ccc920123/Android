...
import com.jysd.toypop.presenter.ArticleJokeNeihanFragmentPresenter;
import com.jysd.toypop.utils.NetWorkUtil;
import com.jysd.toypop.view.holder.ArticleJokeNeihanHolder;
 ...
 */
public class ArticleJokeNeihanFragment extends BaseFragment implements IArticleFragmentView, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void setAdapter(List<Lz13> list) {
        ...
        if (pageNo < pageSize)
            canLoadMore = false;
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter(list, R.layout.fragment_jokeneihan_item, ArticleJokeNeihanHolder.class);//该处需要改
    ...
    @Override
    public ArticleJokeNeihanFragmentPresenter getPresenter() {
        return new ArticleJokeNeihanFragmentPresenter();
    }
    ...
    public void bindView(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(mResId) || mPresenter == null || !(mPresenter instanceof ArticleJokeNeihanFragmentPresenter)) {
            return;
        }
        fl_loading.withLoadedEmptyText("≥﹏≤ , 连条毛都没有 !").withEmptyIco(R.mipmap.note_empty).withBtnEmptyEnnable(false)
            ...
            @Override
            public void onRetry() {
                ((ArticleJokeNeihanFragmentPresenter) mPresenter).getArticles(params);
            }
        }).build();
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (canLoadMore)
                    ArticleJokeNeihanFragment.this.onScrolled(mRecyclerView, dx, dy);
            }
        });
        mRecyclerView.setRecyclerListener(new RecyclerView.RecyclerListener() {
            @Override
            public void onViewRecycled(RecyclerView.ViewHolder holder) {
                NiceVideoPlayer niceVideoPlayer = ((ArticleJokeNeihanHolder) holder).videoPlayer;
        ...
        ((ArticleJokeNeihanFragmentPresenter) mPresenter).getArticles(params);//搭建桥梁
    }
    ...
    private void loadPage() {
        ...
        params.put("url", mResId + "&page=" + (++page + 1));
        params.put("page", String.valueOf(page));
        ((ArticleJokeNeihanFragmentPresenter) mPresenter).getArticles(params);
