 ...
 * 这里处理数据
 */
public class WenXinWuFragmentModel implements IArticleFragmentModel {
    @Override
        ...
        Observable.create(new Observable.OnSubscribe<List<Lz13>>() {
                              ...
                              public void call(Subscriber<? super List<Lz13>> subscriber) {
                                  try {
                                      ...
                                      String scriptJson=result.toString();

                                      String jsonData=scriptJson.substring(scriptJson.indexOf("msgList")+3,scriptJson.length());
