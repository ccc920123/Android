...
import com.jysd.toypop.presenter.WeiXinWuFragmentPresenter;
import com.jysd.toypop.utils.NetWorkUtil;
import com.jysd.toypop.view.holder.WeiXinWuHolder;
...
public class WeiXinWuFragment  extends BaseFragment implements IArticleFragmentView, SwipeRefreshLayout.OnRefreshListener {
    ...
    @Override
    public void setAdapter(List<Lz13> data) {
//        if (mRecyclerView == null) return;
//          pageNo = data.size();
//        if (pageNo < pageSize)
//            canLoadMore = false;
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter(data, R.layout.fragment_weixinwu_item, WeiXinWuHolder.class);//该处需要改

            mRecyclerView.setAdapter(mAdapter);
        } else {
            if ((mAdapter.getItem(0) == null) && (data.size() == 0))
                return;
            if ((mAdapter.getItem(0) == null) || (data.size() == 0) || (!((Lz13) mAdapter.getItem(0)).href.equals(data.get(0).href)))
...
//        }
    ...
    @Override
    public WeiXinWuFragmentPresenter getPresenter() {
        return new WeiXinWuFragmentPresenter();
    }
    ...
    public void bindView(Bundle savedInstanceState) {
        if (TextUtils.isEmpty(mResId) || mPresenter == null || !(mPresenter instanceof WeiXinWuFragmentPresenter)) {
            return;
        }
        fl_loading.withLoadedEmptyText("≥﹏≤ , 连条毛都没有 !").withEmptyIco(R.mipmap.note_empty).withBtnEmptyEnnable(false)
            ...
            @Override
            public void onRetry() {
                ((WeiXinWuFragmentPresenter) mPresenter).getArticles(params);
            }
        }).build();
...
//                if (canLoadMore)
//                    WeiXinWuFragment.this.onScrolled(mRecyclerView, dx, dy);
            }
        });
...
//        params.put("page", String.valueOf(page));
        ((WeiXinWuFragmentPresenter) mPresenter).getArticles(params);//搭建桥梁
    }
