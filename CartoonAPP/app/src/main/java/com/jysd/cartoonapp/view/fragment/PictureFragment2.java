...
import com.jysd.cartoonapp.presenter.PicturePresenter2;
import com.jysd.cartoonapp.utils.NetWorkUtil;
import com.jysd.cartoonapp.view.holder.PictureHolder2;
 ...
 * 图片fragment
 */
public class PictureFragment2 extends BaseFragment implements IPictureView, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.recycler_view)
    public RecyclerView mRecyclerView;

    private BaseRecyclerAdapter<Juzimi> mAdapter;
    ...
    public void setAdapter(List<Juzimi> list) {
        ...
        if (pageNo < pageSize)
            canLoadMore = false;
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter<Juzimi>(list, R.layout.fragment_picture_item, PictureHolder2.class);
            mRecyclerView.setAdapter(mAdapter);
        } else {
            if ((mAdapter.getItem(0) == null) && (list.size() == 0))
                return;
            if ((mAdapter.getItem(0) == null) || (list.size() == 0) || (!(mAdapter.getItem(0)).url.equals(list.get(0).url)))
    ...
    @Override
    public PicturePresenter2 getPresenter() {
        return new PicturePresenter2();
    }
    ...
    public void bindView(Bundle savedInstanceState) {
        if (mPresenter == null || !(mPresenter instanceof PicturePresenter2)) {
            return;
        }
        fl_loading.withLoadedEmptyText("≥﹏≤ , 连条毛都没有 !").withEmptyIco(R.mipmap.disk_file_filter_pic_no_data).withBtnEmptyEnnable(false)
            ...
            @Override
            public void onRetry() {
                ((PicturePresenter2) mPresenter).getJuzimiPictrues(params);
            }
        }).build();
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (canLoadMore)
                    PictureFragment2.this.onScrolled(recyclerView, dx, dy);
            }
        });
        ...
        ((PicturePresenter2) mPresenter).getJuzimiPictrues(params);////搭建桥梁
    }
    ...
    private void loadPage() {
        String loadurl = mUrl.substring(0, mUrl.lastIndexOf("_")+1);
        params.put("url", loadurl + String.valueOf(++page)+".html");
