package com.example.chenpan.library.holder;
...
import com.example.chenpan.library.util.PressedEffectStateListDrawable;
...
public class ImageHolder {
     ...
     */
    public boolean applyTo(ImageView imageView, ImageHolder imageHolder) {
     ...
     * @return true if an image was set
     */
    public boolean apply(ImageHolder imageHolder, ImageView imageView, String tag) {
        if (imageHolder.getUri() != null) {
            if ("http".equals(imageHolder.getUri().getScheme()) || "https".equals(imageHolder.getUri().getScheme())) {
                Picasso.with(imageView.getContext()).load(imageHolder.getUri()).into(imageView);
            } else {
                imageView.setImageURI(imageHolder.getUri());
            }
        } else if (imageHolder.getIcon() != null) {
            imageView.setImageDrawable(imageHolder.getIcon());
        } else if (imageHolder.getBitmap() != null) {
            imageView.setImageBitmap(imageHolder.getBitmap());
        } else if (imageHolder.getIconRes() != -1) {
            imageView.setImageResource(imageHolder.getIconRes());
        } else if (imageHolder.getIIcon() != null) {
            imageView.setImageDrawable(new IconicsDrawable(imageView.getContext(), imageHolder.getIIcon()).actionBar());
        } else {
    ...
    public static boolean applyTo(ImageHolder imageHolder, ImageView imageView, String tag) {
        if (imageHolder != null && imageView != null) {
            return imageHolder.apply(imageHolder, imageView, tag);
        }
        return false;
    }
